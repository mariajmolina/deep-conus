...
#Script for computing updraft helicity (UH; 2-5-km) from WRF CONUS1 simulations (4-km).
...
def open_files(year, month):
    ...
    local, cluster: whether running on mac or cheyenne (boolean). 
            Default is mac.
    ...
    data_wstag = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/CTRL3D/{year}/wrf3d_d01_CTRL_W_{year}{month}*.nc', 
...
def iterate_compute(mapfc):
    ...
    for year in years:
        for month in months:
            ...
            r.to_dataset(name='uh').to_netcdf(f"/glade/scratch/molina/WRF_CONUS1_derived/current/wrf2d_UH_{year}{month}.nc")
