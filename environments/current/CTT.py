...
#Script to compute cloud top temperature from environmental data (WRF CONUS1 4-km simulations).
...
def open_files(year, month):
    ...
    data_cloudstag = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/CTRL3D/{year}/wrf3d_d01_CTRL_QCLOUD_{year}{month}.nc', 
    ...
    data_icestag = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/CTRL3D/{year}/wrf3d_d01_CTRL_QICE_{year}{month}.nc', 
...
years = np.array(list(map(formatter, np.arange(2001,2012,1))))
...
for year in years:
    for month in months:
        ...
        print(f"Saving file")
        r.to_dataset(name='ctt').to_netcdf(f"/glade/scratch/molina/WRF_CONUS1_derived/current/wrf2d_CTT_{year}{month}.nc")
