...
#Script to compute storm relative helicity (SRH 0-3-km) from 
...
def main():
    ...
    cluster
    #view the dask worker job script
    print(cluster.job_script())
    #create client for viewing jobs
    client = Client(cluster)
    client

    #temporal arrays to loop months of data
    ...
    for year in years:
        for month in months:
            ...
            r.to_dataset(name='srh03').to_netcdf(f"/glade/scratch/molina/WRF_CONUS1_derived/future/wrf2d_03SRH_{year}{month}.nc")
...
def open_files(year, month):
    ...
    #geopotential heights unstaggered (m)
...
def wrf_srh(data_ustag, data_vstag, data_zstag, data_mapfc, top=3000.0, lats=None, meta=True):
    ...
    meta: keep attributes and output an xarray dataset.
