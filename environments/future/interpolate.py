...
def open_files(year, month, variable):
    ...
    """
    Helper function to open data sets for interpolation calculation for future climate.
    ...
    if variable == 'TK':
        #temperature (kelvin)
        data_var = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW3D/{year}/wrf3d_d01_PGW_TK_{year}{month}*.nc',
                                       combine='by_coords', parallel=True, chunks={'Time':1}).TK
    if variable == 'QVAPOR':
        #water vapor mixing ratio (kg/kg)
        data_var = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW3D/{year}/wrf3d_d01_PGW_QVAPOR_{year}{month}*.nc',
                                       combine='by_coords', parallel=True, chunks={'Time':1}).QVAPOR      
    if variable == 'EU':
        #earth rotated u-winds (m/s)
        data_var = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW3D/{year}/wrf3d_d01_PGW_EU_{year}{month}*.nc', 
                                       combine='by_coords', parallel=True, chunks={'Time':1}).EU
    if variable == 'EV':
        #earth rotated v-winds (m/s)
        data_var = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW3D/{year}/wrf3d_d01_PGW_EV_{year}{month}*.nc', 
                                       combine='by_coords', parallel=True, chunks={'Time':1}).EV
    if variable == 'P':
        #pres_hpa: full pressure (perturb and base state) #convert from Pa to hPa.
        data_var = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW3D/{year}/wrf3d_d01_PGW_P_{year}{month}*.nc',
                                       combine='by_coords', parallel=True, chunks={'Time':1}).P*0.01 
    if variable == 'QGRAUP':
        #Graupel mixing ratio (kg/kg)
        data_var = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW3D/{year}/wrf3d_d01_PGW_QGRAUP_{year}{month}.nc', 
...
def main():
    ...
    months = np.array(list(map(formatter, np.arange(2,10,1))))
    years = np.array(list(map(formatter, np.arange(2013,2014,1))))
    ...
    for year in years:
        for month in months:
            ...
            print(f"Saving file")
            r.to_dataset(name='levels').to_netcdf(f"/glade/scratch/molina/WRF_CONUS1_derived/future/wrf2d_interp_{variable}_{year}{month}.nc")
