...
#Script to compute maximum CAPE, maximum CIN, LCL, 
#and LFC from environmental data (WRF CONUS1 4-km simulations) 
#during future climate.
...
def main():
    ...
    #temporal arrays for looping through months; takes too many nodes to load full year and computations can be
    #lost if any job failure occurs during the full year computation. Monthly computes is a safer/more resourceful 
    #approach, for now.
    ...
    for year in years:
        for month in months:
            ...
            print(f"Saving file")
            #make sure to not name the variable the same name as a dimension, xarray freaks out about that silently,
            #issue arises upon opening the data set with xarray.
            r.to_dataset(name='cape_cin_lcl_lfc').to_netcdf(f"/glade/scratch/molina/WRF_CONUS1_derived/future/wrf2d_CAPE_{year}{month}.nc")
...
def open_files(year, month):
    ...
    data_sstag = xr.open_mfdataset(f'/gpfs/fs1/collections/rda/data/ds612.0/PGW/{year}/wrf2d_d01_PGW_PSFC_{year}{month1}-{year}{month2}*.nc', 
