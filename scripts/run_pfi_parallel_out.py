...
parser.add_argument("--pfi_variable", required=True, type=int, help="This is variable index for permutation.")
parser.add_argument("--pfi_iterations", required=True, type=int, help="This is number of iterations. Recommend 20.")
...
file=EvaluateDLModel(climate=which_climate, method='random', variables=np.array(['TK','EV','EU','QVAPOR','PRESS']), 
                         ...
                         mask=False, mask_train=False, unbalanced=True, isotonic=False, bin_res=0.1,
                         random_choice=which_random, 
                         obs_threshold=0.5, print_sequential=False, 
                         perm_feat_importance=True, pfi_variable=args.pfi_variable, pfi_iterations=args.pfi_iterations, 
                         currenttrain_futuretest=True, seed_indexer=args.seed_indexer, outliers=True)

test,label=file.intro_sequence_evaluation()
file.solo_pfi(test,label)
